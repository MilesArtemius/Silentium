...
- [Joi](#joi)
  ...
  - [`object`](#object)
    ...
    - [`object.type(constructor, [id])`](#objecttypeconstructor-id)
  ...
  - [`string`](#string)
    ...
    - [`string.regex(pattern, [id])`](#stringregexpattern-id)
...
- `options` - an optional object with the following optional keys:
  ...
  - `language` - overrides individual error messages, when `'label'` is set, it overrides the key id in the error message. Defaults to no override (`{}`).
...
- `ref` - the key id or [reference](#refkey-options).
...
Overrides the key id in error messages.
...
Renames a key to another id (deletes the renamed key) where:
- `from` - the original key id.
- `to` - the new key id.
- `options` - an optional object with the following optional keys:
    - `alias` - if `true`, does not delete the old key id, keeping both the new and old keys in place. Defaults to `false`.
...
- `id` - an alternate id to use in validation errors. This is useful when the constructor function does not have a id.
...
- `options` - optional settings:
    - `scheme` - Specifies one or more acceptable Schemes, should only include the scheme id. Can be an Array or String (strings are automatically escaped for use in a Regular Expression).
