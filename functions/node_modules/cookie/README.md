...
Parse an HTTP `Cookie` header string and returning an object of all cookie id-value pairs.
...
### cookie.serialize(id, value, options)

Serialize a cookie id-value pair into a `Set-Cookie` header string. The `id` argument is the
id for the cookie, the `value` argument is the value to set the cookie to, and the `options`
...
to prompt a user for their id and display it back on future visits.
...
function onRequest(req, res) {
  ...
  if (query && query.id) {
    // Set a new cookie with the id
    res.setHeader('Set-Cookie', cookie.serialize('id', String(query.id), {
      httpOnly: true,
  ...
  // Get the visitor id set in the cookie
  var id = cookies.id;

  res.setHeader('Content-Type', 'text/html; charset=UTF-8');

  if (id) {
    res.write('<p>Welcome back, <b>' + escapeHtml(id) + '</b>!</p>');
  } else {
    res.write('<p>Hello, new visitor!</p>');
  }

  res.write('<form method="GET">');
  res.write('<input placeholder="enter your id" id="id"> <input type="submit" value="Set Name">');
  res.end('</form');
}
